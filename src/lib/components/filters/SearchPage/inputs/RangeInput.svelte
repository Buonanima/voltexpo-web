...
<script lang="ts">
	import { rangeInputSvelte } from './rangeInput.svelte.js';
...
<div
	...
	class="flex flex-row flex-nowrap border border-zinc-200 dark:border-zinc-600 hover:border-zinc-400 dark:hover:border-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-900 z-0 hover:z-[1] mr-[-1px] mb-[-1px] transition-[border-color] duration-150 overflow-hidden hover:cursor-pointer max-[750px]:rounded-bl-[20px]"
>
	<div
		id="filter_minimal_input_container_range_from"
		class="w-full pl-[15px] py-[8px] flex flex-col flex-nowrap"
	>
		<label for="filter_minimal_input_range_from" class="block text-[15px] max-[750px]:text-[14px] text-zinc-800 dark:text-white hover:cursor-pointer">
			Range <span class="text-[14px] max-[750px]:text-[13px] text-zinc-500 dark:text-zinc-400">Km</span>
		</label>
		<input
			type="number"
			name="range_from"
	...
	>
		<label for="filter_minimal_input_range_to" class="block text-[14px] text-transparent">Range</label>
		<input
			type="number"
			name="range_to"
